...
Route::get('albaranes', function () {
	$albaranes = Albaran::orderBy('Id', 'desc')->with('detallealbaran')->get();
...
Route::get('albaranesporcliente/{id}', function ($id) {
	...
	foreach ($avisos as $key => $aviso) {
		$albaranes[$key] = Albaran::where('aviso_id', $aviso->id)->with('detallealbaran')->get();
	}
...
Route::get('albaranesporaviso/{id}', function ($id) {
	$albaranes = Albaran::where('aviso_id', $id)->orderBy('aviso_id', 'desc')->get();
	return json_encode($albaranes);
});
...
Route::put('empleados/{id}', function (Request $request, $id) {
	...
	DB::table('empleados')
		->where('id', $id)
		->update(array('name' => $request->name, 'telefono' => $request->telefono, 'activo' => $request->activo));
