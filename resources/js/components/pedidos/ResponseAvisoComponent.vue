<template>
  <tr>
    ...
    <td class="text-center">
      ...
      <div v-else class="dropdown">
        ...
        <div class="dropdown-menu " aria-labelledby="dropdownMenu2">
          ...
          >{{empleado.name}}</button>
        </div>
      </div>
    </td>
    <td class="text-center">
      <p v-if="!flageditar">{{ aviso.fechaPrevista | moment("DD/MM/YYYY, h:mm a")}}</p>
      <input v-else type="datetime-local" class="form-control" v-model="aviso.fechaPrevista" />
    </td>
    <td class="text-center">
      <p v-if="!flageditar">{{ aviso.comentario}}</p>
      <input v-else type="text" class="form-control" v-model="aviso.comentario" />
    </td>

    <td class="text-center">
      <div class="btn" v-bind:class="[ aviso.terminada ? terminado : noterminado]"></div>
    </td>
    <td>
    ...
    <modal-component :key="showModal" v-if="showModal" @close="showModal = false" :avis="aviso" :nombre="nombre"></modal-component>
  </tr>
</template>
