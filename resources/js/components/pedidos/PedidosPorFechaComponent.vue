...
export default {
  ...
  methods: {
      ...
      if (this.tipoFecha) {
        if (this.tipoFecha == "Fecha Prevista") {
          this.avisosbuscados = this.avisos.filter((ax) => {
            return (
              moment(Date.parse(ax.fechaPrevista)).format("yyyyMMDD") >=
                moment(Date.parse(this.datefrom)).format("yyyyMMDD") &&
              moment(Date.parse(ax.fechaPrevista)).format("yyyyMMDD") <=
                moment(Date.parse(this.dateto)).format("yyyyMMDD") &&
              ax.fechaPrevista
            );
          });
        } else {
          this.avisosbuscados = this.avisos.filter((ax) => {
            return (
              moment(Date.parse(ax.created_at)).format("yyyyMMDD") >=
                moment(Date.parse(this.datefrom)).format("yyyyMMDD") &&
              moment(Date.parse(ax.created_at)).format("yyyyMMDD") <=
                moment(Date.parse(this.dateto)).format("yyyyMMDD")
            );
    ...
    setPages() {
      this.pages = [];
      let numberOfPages = Math.ceil(this.avisosbuscados.length / this.perPage);
